...
-spec info(Beam) -> [InfoPair] | {'error', 'beam_lib1', info_rsn()} when
      Beam :: beam(),
...
-spec all_chunks(beam()) -> {'ok', 'beam_lib1', [{chunkid(), dataB()}]}.

all_chunks(File) ->
    read_all_chunks(File).

-spec cmp(Beam1, Beam2) -> 'ok' | {'error', 'beam_lib1', cmp_rsn()} when
      Beam1 :: beam(),
      Beam2 :: beam().
...
-spec cmp_dirs(Dir1, Dir2) ->
           {Only1, Only2, Different} | {'error', 'beam_lib1', Reason} when
...
-spec diff_dirs(Dir1, Dir2) -> 'ok' | {'error', 'beam_lib1', Reason} when
      Dir1 :: atom() | file:filename(),
      Dir2 :: atom() | file:filename(),
      Reason :: {'not_a_directory', term()} | info_rsn().
...
-spec strip(Beam1) ->
        {'ok', {module(), Beam2}} | {'error', 'beam_lib1', info_rsn()} when
      Beam1 :: beam(),
      Beam2 :: beam().
...
-spec strip_files(Files) ->
        {'ok', [{module(), Beam}]} | {'error', 'beam_lib1', info_rsn()} when
      Files :: [beam()],
      Beam :: beam().
...
-spec md5(Beam) ->
        {'ok', {module(), MD5}} | {'error', 'beam_lib1', chnk_rsn()} when
      Beam :: beam(),
      MD5 :: binary().
...
compare_files([{_,F1} | R1], [{_,F2} | R2], Acc) ->
    %io:format("\n F1 is ~p \n F2 is ~p ~n Acc is ~p~n",[F1,F2,Acc]),
    NAcc = case catch cmp_files(F1, F2) of
	       {error, _Mod, _Reason} ->
		   [{F1, F2} | Acc];
	       ok ->
                 %io:format("\n Inside Acc ~n Acc is ~p~n",[Acc]),
		   Acc
	   end,
...
cmp_lists([{Id, C1} | R1], [{Id, C2} | R2]) ->
    if
	Id =:= "Abst" ->
           %io:format("~n Inside Abst ~n"),
	    cmp_lists(R1, R2);
	C1 =:= C2 ->
	    %io:format("\n Id is ~p \n",[Id]),
	    cmp_lists(R1, R2);
	true ->
           %io:format("~n Inside true ~n Id value is ~p ~n",[Id]),
...
-define(CRYPTO_KEY_SERVER, beam_lib1__crypto_key_server).
